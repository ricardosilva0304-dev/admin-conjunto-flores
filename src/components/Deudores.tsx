    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-20 px-2 md:px-0 font-sans text-slate-800">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="bg-slate-900 p-6 rounded-xl text-white">
          <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Total en Calle</p>
          <h3 className="text-2xl font-black text-emerald-400">
            ${lista.reduce((acc, r) => acc + (r.saldoReal > 0 ? r.saldoReal : 0), 0).toLocaleString('es-CO')}
          </h3>
        </div>
        <div className="bg-white p-6 rounded-xl border">
          <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Unidades en Mora</p>
          <h3 className="text-2xl font-black text-rose-500">{lista.filter(r => r.saldoReal > 0).length}</h3>
        </div>
        <div className="hidden md:flex bg-white p-6 rounded-xl border items-center justify-between">
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Paz y Salvo</p>
      ...
      <div className="space-y-3">
        {lista.map(res => (
          <div key={res.id} className="bg-white border border-slate-100 p-5 rounded-2xl flex flex-col md:flex-row items-center justify-between hover:border-emerald-200 transition-all">
            <div className="flex items-center gap-5 md:w-1/3">
              ...
              <div>
                ...
                <p className={`text-[9px] font-black uppercase tracking-widest ${res.saldoReal > 0 ? "text-rose-400" : "text-emerald-500"}`}>
            ...
            <div className="flex gap-2">
              {/* ESTE BOTÓN SIEMPRE ESTÁ ACTIVO */}
              <button 
                onClick={() => setResidenteDetalle(res)} 
                className="px-5 py-3 border border-slate-200 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all flex items-center gap-2"
              >
                <FileText size={14}/> Estado Cuenta
              </button>

              {/* ESTE BOTÓN SOLO SI DEBE DINERO */}
              <button 
                onClick={() => setCobroResidente(res)} 
                className={`px-5 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${res.saldoReal > 0 ? "bg-slate-900 text-white shadow-lg" : "bg-slate-50 text-slate-200 pointer-events-none"}`}
              >
                Cuenta Cobro
              </button>
            </div>
