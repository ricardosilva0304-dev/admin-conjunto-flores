...
export default function Resumen({ adminName, goToDeudores }: ResumenProps) {
    ...
    async function cargarDatosDashboard() {
        ...
        try {
            ...
            // LÓGICA DE CAJA REAL: Ingresos en Efectivo - Gastos Totales
            const porEfectivoBruto = pagosRes.data?.filter(p => p.metodo_pago === 'Efectivo').reduce((acc, p) => acc + Number(p.monto_total), 0) || 0;
            const efectivoEnCajaReal = porEfectivoBruto - egrTotal;
            ...
            setStats({
                ...
                banco: porBanco,
                efectivo: efectivoEnCajaReal
            });
            setMorosos(Object.values(deudaPorResidente).sort((a: any, b: any) => b.totalDeuda - a.totalDeuda).slice(0, 5));
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 pb-24 font-sans px-2 md:px-0 animate-in fade-in duration-1000">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-slate-900 text-4xl md:text-5xl font-black tracking-tighter leading-none mb-2 italic uppercase">Hola, {adminName.split(' ')[0]}</h1>
                    <div className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] flex items-center gap-2">
                        <div className="w-6 h-[1.5px] bg-emerald-500"></div> Estado financiero real
                    </div>
                </div>
                <div className="bg-slate-900 text-white px-5 py-3 rounded-xl flex items-center gap-4 shadow-xl">
                    <Calendar size={18} className="text-emerald-400" />
                    <p className="text-[11px] font-black uppercase tracking-widest">{new Intl.DateTimeFormat('es-CO', { month: 'long', year: 'numeric' }).format(new Date())}</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    { label: 'Egresos (Efectivo)', val: stats.egresosMes, icon: <TrendingDown size={20} />, c: 'rose' },
                    { label: 'Balance Neto', val: stats.balanceMes, icon: <Wallet size={20} />, c: 'blue' },
                    { label: 'Cartera Total', val: stats.carteraPendiente, icon: <AlertTriangle size={20} />, c: 'rose' },
                ].map((item, i) => (
                    <div key={i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-4 ${item.c === 'emerald' ? 'bg-emerald-50 text-emerald-600' : item.c === 'rose' ? 'bg-rose-50 text-rose-600' : 'bg-blue-50 text-blue-600'}`}>
                            {item.icon}
                        </div>
                        <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mb-1">{item.label}</p>
                        <h3 className={`text-2xl font-black tabular-nums ${item.label === 'Cartera Total' ? 'text-rose-600' : 'text-slate-800'}`}>${item.val.toLocaleString('es-CO')}</h3>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <div className="lg:col-span-4 bg-white border border-slate-100 rounded-[2rem] p-8 flex flex-col justify-between shadow-sm">
                    <div>
                        <h3 className="text-slate-800 font-bold text-sm uppercase tracking-widest flex items-center gap-2 mb-8"><LayoutGrid size={16} className="text-slate-400" /> Arqueo de Fondos</h3>
                        <div className="space-y-6">
                            <div className="flex justify-between items-center border-b border-slate-50 pb-4">
                                <span className="text-[11px] font-bold text-slate-500 uppercase">En Banco</span>
                                <span className="font-black text-slate-900 tabular-nums">${stats.banco.toLocaleString('es-CO')}</span>
                            </div>
                            <div className="flex justify-between items-center pb-4">
                                <span className="text-[11px] font-bold text-slate-500 uppercase">Efectivo en Caja</span>
                                <span className={`font-black tabular-nums ${stats.efectivo < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>${stats.efectivo.toLocaleString('es-CO')}</span>
                            </div>
                        </div>
                    </div>
                    <div className="bg-emerald-50 p-6 rounded-2xl text-center border border-emerald-100 mt-6">
                        <p className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-2">Eficiencia de Recaudo</p>
                        <div className="text-3xl font-black text-emerald-800">{stats.ingresosMes + stats.carteraPendiente > 0 ? Math.round((stats.ingresosMes / (stats.ingresosMes + stats.carteraPendiente)) * 100) : 0}%</div>
                    </div>
                </div>
                ...
                <div className="lg:col-span-8 bg-white border border-slate-100 rounded-[2rem] p-8 shadow-sm">
                    <h3 className="text-slate-800 font-bold text-sm uppercase tracking-widest flex items-center gap-2 mb-8">
                        <div className="w-1.5 h-6 bg-rose-600 rounded-full"></div> Mayores Deudores
                    </h3>
                        ...
                        {morosos.map((m, i) => (
                            <div key={i} className="flex items-center justify-between p-4 bg-slate-50/50 border border-slate-100 rounded-2xl">
                                <div className="flex items-center gap-5">
                                    ...
                                    <div className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center font-black text-[10px]">{m.unidad}</div>
                                    <div>
                                        <p className="text-xs font-black text-slate-800 uppercase">{m.nombre}</p>
                                        <p className="text-[9px] font-bold text-slate-400 uppercase">{m.cantidadRecibos} conceptos en mora</p>
                                    </div>
                                </div>
                                <p className="text-sm font-black text-rose-600 tabular-nums">${m.totalDeuda.toLocaleString('es-CO')}</p>
                            </div>
                        ))}
                    </div>
                    <button onClick={goToDeudores} className="w-full mt-8 py-4 bg-slate-50 text-slate-500 font-black text-[10px] uppercase tracking-[0.3em] rounded-xl hover:bg-slate-900 hover:text-white transition-all flex items-center justify-center gap-2">
