...
export default function App() {
  ...
  const [loadingSession, setLoadingSession] = useState(true); // evita flash de login
  ...
  useEffect(() => {
    // Verifica si ya hay una sesión activa (ej: recarga de página)
  ...
  // Carga nombre y rol desde la tabla perfiles_admin usando el UUID del usuario Auth
  const cargarPerfilUsuario = async (userId: string) => {
    const { data } = await supabase
      .from("perfiles_admin")
      .select("nombre, rol")
      .eq("auth_user_id", userId)  // nueva columna que reemplaza cédula+password
      .single();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Si tiene éxito, onAuthStateChange se dispara y actualiza el estado
    } catch {
  ...
  const sectionMeta: Record<string, { label: string; labelShort: string; icon: React.ReactNode }> = {
  ...
  // ── Pantalla de carga inicial (evita flash de login si ya hay sesión) ──
  if (loadingSession) {
    return (
        ...
        <div className="w-full max-w-[420px] z-10 animate-in fade-in zoom-in-95 duration-700">
          <div className="flex flex-col items-center mb-10">
            <div className="w-20 h-20 bg-gradient-to-tr from-emerald-600 to-emerald-400 rounded-[2rem] flex items-center justify-center shadow-2xl shadow-emerald-500/20 mb-8 rotate-3 transition-all hover:rotate-0 duration-500">
              <Lock className="text-black" size={36} strokeWidth={2.5} />
            </div>
          ...
          <div className="bg-zinc-900/40 backdrop-blur-3xl border border-white/5 p-10 rounded-[3rem] shadow-2xl">
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="space-y-2">
                <label className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em] ml-2">Correo</label>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-black py-5 rounded-2xl transition-all shadow-xl active:scale-95 disabled:opacity-30 uppercase tracking-widest text-xs"
              >
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              <div className="flex items-baseline gap-1 text-slate-900">
                <span className="font-black tabular-nums tracking-tighter text-lg md:text-2xl">
              ...
              <div className="ml-1 md:ml-3 pl-2 md:pl-4 border-l border-slate-100">
                <div className="px-1.5 md:px-2 py-0.5 bg-slate-50 border border-slate-100 rounded text-[8px] md:text-[9px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                  ...
                  <span className="hidden sm:inline">
                    {currentTime.toLocaleDateString('es-CO', { weekday: 'short', day: '2-digit', month: 'short' }).toUpperCase()}
                  </span>
                </div>
              </div>
            </div>
